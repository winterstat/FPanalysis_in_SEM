...
ggsave(plot = plot_1, path = here::here("Online-Supplement","Plots", "Study Two"), 
       filename = 'max-iterations-fp-study-2-condition_1-model-3A.png', width = 10, height = 8,
       units = 'in')
...
data_results |>
  ...
  ggplot(aes(x = max_iter, y = values)) +
  geom_point() + geom_line(color = "#276419") + labs(x = "Maximum number of iterations", y = "LL > Cutoff") +
  scale_x_continuous(breaks = seq(150, 1.5e4, 500)) -> plot_2


# saving the plots 
ggsave(plot = plot_2, path = here::here("Online-Supplement","Plots", "Study Two"), 
       filename = 'max-iterations-fp-study-2-condition_2-model-3A.png', width = 10, height = 8,
       units = 'in')
...
ggsave(plot = plot_3, path = here::here("Online-Supplement","Plots", "Study Two"), 
       filename = 'max-iterations-fp-study-2-condition_3-model-3A.png', width = 10, height = 8,
       units = 'in')
...
data_results |>
  ...
  ggplot(aes(x = max_iter, y = values)) +
  geom_point() + geom_line(color = "#276419") + labs(x = "Maximum number of iterations", y = "LL > Cutoff") +
  scale_x_continuous(breaks = seq(150, 1.5e4, 500)) -> plot_4


# saving the plots 
ggsave(plot = plot_4, path = here::here("Online-Supplement","Plots", "Study Two"), 
       filename = 'max-iterations-fp-study-2-condition_4-model-3A.png', width = 10, height = 8,
       units = 'in')
