...
model_3c_results_2 <- readRDS(here::here("Online-Supplement","Data-Files" ,"model_3C_fp_con_2.rds"))
model_3c_results_3 <- readRDS(here::here("Online-Supplement","Data-Files" ,"model_3C_fp_con_3.rds"))
model_3c_results_4 <- readRDS(here::here("Online-Supplement","Data-Files" ,"model_3C_fp_con_4.rds"))

# cut points
ll_cufoff_1 <- -62493

# Fit propensity across the maximum number of iterations
fp_values_1 <- sapply(model_3c_results_1, function(x){
...
fp_values_2 <- sapply(model_3c_results_2, function(x){
...
fp_values_3 <- sapply(model_3c_results_3, function(x){
...
fp_values_4 <- sapply(model_3c_results_4, function(x){
...
data_results |>
  ...
  annotate("text", x = 2700, y = Inf, label = "At max.iter = 1500",vjust = 7.5, color = "grey40") -> plot_1


# saving the plots 
ggsave(plot = plot_1, path = here::here("Online-Supplement","Plots", "Study Three"), 
       filename = 'max-iterations-fp-study-3-condition_1-model-3C.png', width = 10, height = 8,
       units = 'in')
...
data_results |>
  ...
  annotate("text", x = 7000, y = Inf, label = "At max.iter = 6000",vjust = 7.5, color = "grey40")  -> plot_2


# saving the plots 
ggsave(plot = plot_2, path = here::here("Online-Supplement","Plots", "Study Three"), 
       filename = 'max-iterations-fp-study-3-condition_2-model-3C.png', width = 10, height = 8,
       units = 'in')
...
data_results |>
  ...
  scale_x_continuous(breaks = seq(150, 1.5e4, 500)) +
  geom_vline(xintercept = 1500, color = "grey67", linetype = 'dashed', linewidth = 1) +
  annotate("text", x = 2700, y = Inf, label = "At max.iter = 1500",vjust = 6.5, color = "grey40") -> plot_3


# saving the plots 
ggsave(plot = plot_3, path = here::here("Online-Supplement","Plots", "Study Three"), 
       filename = 'max-iterations-fp-study-3-condition_3-model-3C.png', width = 10, height = 8,
       units = 'in')
...
data_results |>
  ...
  geom_point() + geom_line(color = "#276419") + labs(x = "Maximum number of iterations", y = "Fitting Propensity") +
  scale_x_continuous(breaks = seq(150, 1.5e4, 500)) +
  geom_vline(xintercept = 2500, color = "grey67", linetype = 'dashed', linewidth = 1) +
  annotate("text", x = 3500, y = Inf, label = "At max.iter = 2500",vjust = 7.5, color = "grey40")  -> plot_4


# saving the plots 
ggsave(plot = plot_4, path = here::here("Online-Supplement","Plots", "Study Three"), 
       filename = 'max-iterations-fp-study-3-condition_4-model-3C.png', width = 10, height = 8,
       units = 'in')
