...
fourcond_logl %>% group_by(stratum) %>%
  summarize(
    ...
    fp_refmarker = sum(logl_refmarker > -62493) / n(),
    
    fp_unitvar = sum(logl_unitvar > -62493) / n(),
...
#   1 Always-taker      42746     -62882.   -62887.   -4.82      0.00508    0.00180 -0.00328
# 2 Complier (Lunit) 156273     -62135.   -62902. -767.        0.633      0.00182 -0.631  
# 3 Defier (Lref)      5459     -62770.   -62455.  315.        0.106      0.361    0.254  
# 4 Never-taker      795522     -61998.   -62276. -278.        0.742      0.517   -0.225 

save(logl_all_3, file = here::here("Online-Supplement","Data-Files","logl_all_3B.RDS"))
#-------------------------------------------------------------------------------
