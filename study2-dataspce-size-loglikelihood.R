...
srmr.fp_1 <- function(fits, cutoff = .09) {
...
# saving the graph
ggsave(plot = plot_1, path = here::here("Plots", "Study Two"), 
...
# saving the graph
ggsave(plot = plot_2, path = here::here("Plots", "Study Two"), 
...
cfi.fp <- function(fits, cutoff = .45) {
  ...
  return((sum(fits >= cutoff, na.rm = T) / length(fits)))
}
