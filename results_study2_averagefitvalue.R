...
samfits <- function(fits_all, size = 100) {
  ...
  # renaming columns
  colnames(results) <- c('condition', 'cfi', 'rmsea', 'srmr', 'll')
...
for(i in 1:100) {
  ...
  colnames(tmp) <- c('condition', 'cfi', 'rmsea', 'srmr', 'll', 'size')
  
  if(i == 1) {
    average.size.results <- tmp
  } else {
    average.size.results <- rbind(average.size.results, tmp)
  }
}
...
res_pos <- merge(average.size.results, study2_conditions)

res_pos$model <- factor(res_pos$model)


# saving the results
saveRDS(average.size.results, file = here::here("Online-Supplement","Data-Files", "average_study2_poscor_dataspacesize.RDS"))

saveRDS(res_pos, file = here::here("Online-Supplement","Data-Files", "average_results_poscor_study_2_dataspacesize.RDS"))
...
for(i in 1:100) {
  ...
  colnames(tmp1) <- c('condition', 'cfi', 'rmsea', 'srmr', 'll', 'size')
  
  if(i == 1) {
    average.size.results1 <- tmp1
  } else {
    average.size.results1 <- rbind(average.size.results1, tmp1)
  }
}


# merging the data frame with the conditions
res_all <- merge(average.size.results1, study2_conditions)

res_all$model <- factor(res_all$model)

# saving the results
saveRDS(average.size.results1, file = here::here("Online-Supplement","Data-Files", "average_study2_all_dataspacesize.RDS"))

saveRDS(res_all, file = here::here("Online-Supplement","Data-Files", "average_results_all_study_2_dataspacesize.RDS"))

# merging the two data sets by a common variable called cor

res_pos <- readRDS(here::here("Online-Supplement","Data-Files","average_results_poscor_study_2_dataspacesize.RDS"))

res_all <- readRDS(here::here("Online-Supplement","Data-Files", "average_results_all_study_2_dataspacesize.RDS"))
...
results_full %>%
  pivot_longer(cols = cfi:ll, names_to = "index") %>%
...
plot_1_positive <- results_full %>%
  ...
  mutate(model = factor(model, levels = c(1, 2, 3),  labels = c("Model 1B", "Model 2B", "Model 3B")),
         ...
         index = factor(index, levels = c("cfi", "rmsea", "srmr", "ll"), labels = c("CFI", "RMSEA", "SRMR", "LL")), 
  ...
  scale_fill_manual(name = "Model",
                    values = c("Model 1B" = "#276419",
                               "Model 2B" = "#8e0152", 
                               "Model 3B" = "#b2182b")) + scale_linetype_manual(
    name = "Model",
...
ggsave(plot = plot_1_positive, path = here::here("Online-Supplement","Plots", "Study Two"), 
       filename = 'cfi-average-positive-subspace-unrestricted-bounds-study-2.png', width = 10, height = 8,
       units = 'in')
...
ggsave(plot = plot_1_all, path = here::here("Online-Supplement","Plots", "Study Two"),
       filename = 'cfi-average-complete-space-unrestricted-bounds-study-2.png', width = 10, height = 8,
       units = 'in')
...
ggsave(plot = plot_2_positive, path = here::here("Online-Supplement","Plots", "Study Two"),
       filename = 'srmr-average-positive-subspace-unrestricted-bounds-study-2.png', width = 10, height = 8,
       units = 'in')
...
ggsave(plot = plot_2_all, path = here::here("Online-Supplement","Plots", "Study Two"), 
       filename = 'srmr-average-complete-space-unrestricted-bounds.png-study-2.png', width = 10, height = 8,
       units = 'in')
...
ggsave(plot = plot_3_positive, path = here::here("Online-Supplement","Plots", "Study Two"), 
       filename = 'rmsea-average-positive-subspace-unrestricted-bounds-study-2.png', width = 10, height = 8,
       units = 'in')
...
ggsave(plot = plot_3_all, path = here::here("Online-Supplement","Plots", "Study Two"), 
       filename = 'rmsea-average-complete-space-unrestricted-bounds-study-2.png', width = 10, height = 8,
       units = 'in')